...
2026-08-07: 決定と契約の区別(agents-test-design: 決定は採用・不採用とも台帳行きで検査の根拠にしない、不採用の昇格条件は「契約の 3 形で言える約束+黙って覆る」、frontmatter に観測の瞬間「方針の決定を検査で固定したくなったとき」を追加、対照例 1 件)— は、別プロジェクトのセッションで観測された失敗(席種の列挙検査が意図した追加で落ちる変更検出器化、不採用決定への見張り提案)への対策である。判定手順(赤への応答リトマス)は意図して書いていない — 概念の区別から導出可能な推論レシピは新世代モデルへの縛りになる。効果の観測が付くまでアブレーションの対象にしない。
