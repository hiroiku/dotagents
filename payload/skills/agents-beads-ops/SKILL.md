...
- ぶら下げてよいのは、統合ブランチ上で再現し、かつ**業務の契約を守る検査**の破損・不足だけ。自分のブランチ起因のテスト失敗はその作業の `Blocker` であり、ここに付け替えて merge ゲートを素通りさせない。
- 足場の検査(業務契約を守らない検査・ガード・監視)の破損は子にしない — その場で存在資格を審査し(agents-test-design「壊れた検査に遭遇したら」)、結論(撤去した・沈めた・残す理由)は epic への note に残す。修理の常設集約先を作ると、足場の修理が仕事の主流になる。
