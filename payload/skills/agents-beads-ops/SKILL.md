...
- 同型の子が続くなら、それは個別の破損ではなく検査基盤の `欠陥クラス` である(独立性・後始末・共有状態のどれかの契約が構造で守られていない)。子を 1 件ずつ直し続けず、基盤の契約を 1 つの `欠陥クラス` issue に畳んで構造で直す。test-health が定常的に子を生み続ける状態は、ガードの層が高すぎる合図である(層下げは `agents-test-design`)。
